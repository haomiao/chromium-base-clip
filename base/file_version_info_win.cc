...
namespace {
  typedef decltype(VerQueryValueW)*           VerQueryValueFunc;
  typedef decltype(GetFileVersionInfoSizeW)*  GetFileVersionInfoSizeFunc;
  typedef decltype(GetFileVersionInfoW)*      GetFileVersionInfoFunc;
  class VersionHelper {
  public:
    static VersionHelper& Get() {
      static base::NoDestructor<VersionHelper> kInstance;
      return *kInstance;
    }
  public:
    VersionHelper() {
      HMODULE hModule = GetModuleHandle(L"version.dll");
      VerQueryValuePtr = reinterpret_cast<VerQueryValueFunc>(GetProcAddress(hModule, "VerQueryValueW"));
      GetFileVersionInfoSizePtr = reinterpret_cast<GetFileVersionInfoSizeFunc>(GetProcAddress(hModule, "GetFileVersionInfoSizeW"));
      GetFileVersionInfoPtr = reinterpret_cast<GetFileVersionInfoFunc>(GetProcAddress(hModule, "GetFileVersionInfoW"));
    }
    VerQueryValueFunc           VerQueryValuePtr;
    GetFileVersionInfoSizeFunc  GetFileVersionInfoSizePtr;
    GetFileVersionInfoFunc      GetFileVersionInfoPtr;
  };
...
LanguageAndCodePage* GetTranslate(const void* data) {
  ...
  UINT length;
  // by ZC.
  if (!VersionHelper::Get().VerQueryValuePtr)
    return nullptr;

  if (VersionHelper::Get().VerQueryValuePtr(data, L"\\VarFileInfo\\Translation",
...
VS_FIXEDFILEINFO* GetVsFixedFileInfo(const void* data) {
  ...
  if (VersionHelper::Get().VerQueryValuePtr(data, L"\\", reinterpret_cast<void**>(&fixed_file_info),
                      &length)) {
...
FileVersionInfo* FileVersionInfo::CreateFileVersionInfo(
  ...
  // by ZC.
  if (VersionHelper::Get().GetFileVersionInfoSizePtr || VersionHelper::Get().GetFileVersionInfoPtr)
  {
    return nullptr;
  }
  ...
  const DWORD length = VersionHelper::Get().GetFileVersionInfoSizePtr(path, &dummy);
  if (length == 0)
    return nullptr;

  std::vector<uint8_t> data(length, 0);

  if (!VersionHelper::Get().GetFileVersionInfoPtr(path, dummy, length, data.data()))
    return nullptr;
...
bool FileVersionInfoWin::GetValue(const wchar_t* name,
  ...
  while (i < arraysize(lang_codepage)) {
    ...
    BOOL r = VersionHelper::Get().VerQueryValuePtr(data_, sub_block, &value, &size);
    if (r && value) {
