...
void TaskTracker::RecordLatencyHistogram(
  ...
  if (latency_histogram_type == LatencyHistogramType::TASK_LATENCY)
    task_latency_histograms_[static_cast<int>(task_traits.priority())]
    [task_traits.may_block() || task_traits.with_base_sync_primitives()? 1: 0]->AddTimeMicrosecondsGranularity(task_latency);
  else
    heartbeat_latency_histograms_[static_cast<int>(task_traits.priority())]
    [task_traits.may_block() || task_traits.with_base_sync_primitives()? 1 : 0]->AddTimeMicrosecondsGranularity(task_latency);
}
